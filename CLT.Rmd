...
* This papre illustrates these lwas by the example based on the of exponential distribution.
...
```{r}
paste(round(abs(mean(means) - population.mean) * 100, 1), "%", sep = "")
```
...
dat <- data.frame(
  x = c(apply(samples[, 1:10], 1, cfunc, 10),
        apply(samples[, 1:20], 1, cfunc, 20),
